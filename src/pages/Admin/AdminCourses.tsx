...
const AdminCourses = () => {
  ...
  const handleSaveCourse = (e: React.FormEvent) => {
    ...
    setCourses(prev => {
      ...
      if (existingIndex >= 0) {
        const updated = [...prev];
        updated[existingIndex] = { ...updated[existingIndex], ...currentCourse };
        return updated;
      } else {
              ...
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <div className="relative">
                  <input
                    ...
                    className="rounded-lg border border-gray-300 px-3 py-1.5 pr-8 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
                    value={searchQuery}
                ...
                {filteredCourses.map((course) => {
                  ...
                  return (
                    <tr key={course.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-sm text-gray-600">{course.updated}</td>
                      <td className="px-3 py-3">
                        <div className="flex flex-wrap items-center gap-2">
                          <button 
                            ...
                            className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 transition hover:border-brand-400 hover:bg-brand-50"
                          >
                            Chỉnh sửa
                          </button>
                          ...
                          {course.isDraft ? (
                            <button 
                              ...
                              className="rounded-lg border border-green-500 bg-green-50 px-3 py-1.5 text-xs font-medium text-green-700 transition hover:bg-green-100"
                            >
                              Xuất bản
                            </button>
                          ) : (
                            <>
                              <button 
                                onClick={() => toggleCourseStatus(course.id, course.status === 'public' ? 'private' : 'public')}
                                className="rounded-lg border border-blue-500 bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 transition hover:bg-blue-100"
                              >
                                {course.status === 'public' ? 'Chuyển riêng tư' : 'Chuyển công khai'}
                              </button>
                              <button 
                                onClick={() => toggleCourseStatus(course.id, 'paused')}
                                className="rounded-lg border border-yellow-500 bg-yellow-50 px-3 py-1.5 text-xs font-medium text-yellow-700 transition hover:bg-yellow-100"
                              >
                                Tạm dừng
                              </button>
                            </>
                          ...
                          <Link
                            to={`/admin/courses/${course.id}/members`}
