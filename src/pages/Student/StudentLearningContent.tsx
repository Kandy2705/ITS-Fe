    ...
    <>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-card dark:border-gray-800 dark:bg-white/5">
          ...
          <p className="text-sm text-gray-600">Kiểm tra hạn nộp trong danh sách bên dưới</p>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-card dark:border-gray-800 dark:bg-white/5">
          ...
          <p className="text-sm text-gray-600">Có thể xem lại lịch sử và phản hồi</p>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-card dark:border-gray-800 dark:bg-white/5">
          ...
          <p className="mt-2 text-3xl font-bold text-gray-900">
            {(
              currentAssignments
                ...
                currentAssignments.filter((item) => typeof item.score === "number").length
            ).toFixed(1)}
          </p>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-card dark:border-gray-800 dark:bg-white/5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">Xem lại file đã nộp và phản hồi từ giảng viên.</p>
              </div>
              <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">Lưu trữ</span>
            </div>
                ...
                <div key={item.id} className="rounded-xl border border-gray-100 bg-gray-50 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Nộp ngày {item.submittedAt}</p>
                    </div>
                    {item.score && (
                  ...
                  {item.feedback && <p className="mt-1 text-sm text-gray-700">Nhận xét: {item.feedback}</p>}
                </div>
              ))}
            </div>
          </div>
